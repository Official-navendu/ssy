...
import {
  ...
  ChevronDown, Send, Check, Sparkles, AlertCircle, X, MessageSquare
} from "lucide-react";
...
export default function Contact() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const result = await sendEmailParams("CONTACT_FORM", payload);
      if (result.success) {
        setSuccess(true);
        reset();
      } else {
        setErrorMsg(result.error || "Could not submit message. Please try WhatsApp.");
      }
    } catch (err) {
          ...
          {[
            ...
            <Reveal key={c.label} delay={i * 0.08}>
              <a href={c.href} target={c.href.startsWith("http") ? "_blank" : undefined} rel="noreferrer"
                ...
                <div className="absolute -top-12 -right-12 h-24 w-24 rounded-full bg-sapphire/10 blur-xl opacity-40 group-hover:opacity-75 transition-opacity pointer-events-none" />
                
                <div className="flex items-center gap-4">
                  <div className="grid h-12 w-12 place-items-center rounded-xl bg-gradient-to-br from-gold/25 to-sapphire/25 text-gold border border-gold/20 shadow-[0_0_15px_rgba(216,182,122,0.1)] transition-transform duration-500 group-hover:scale-110">
        ...
        <div className="mx-auto grid max-w-6xl gap-12 px-6 md:grid-cols-2 md:px-8 items-stretch">
          ...
          <Reveal>
            <div className="group relative overflow-hidden rounded-3xl glass p-8 md:p-10 border border-gold/15 shadow-[0_20px_50px_rgba(0,0,0,0.6)] bg-card/10 flex flex-col justify-between h-full">
              {/* Soft sapphire-gold background highlights */}
              <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-sapphire/5 via-transparent to-gold/5 opacity-40 -z-10" />
    ...
    <Youtube className="h-4 w-4" />
          ...
          <Reveal delay={0.1}>
            <div className="group relative overflow-hidden rounded-3xl glass p-2 transition-all duration-500 hover:border-gold/30 hover:shadow-[0_15px_45px_rgba(216,182,122,0.08)] bg-card/10 flex flex-col justify-between h-full">
              ...
              <div className="absolute -inset-4 bg-gradient-to-tr from-sapphire/10 to-gold/5 rounded-3xl blur-3xl opacity-60 group-hover:opacity-85 transition-opacity duration-700 pointer-events-none" />
              ...
              <div className="relative z-10 h-full flex flex-col justify-between">
                ...
                <div className="p-4 pt-1">
                  <div className="flex items-start gap-4">
                    <div className="grid h-11 w-11 place-items-center rounded-xl bg-gradient-to-br from-gold/25 to-sapphire/25 text-gold border border-gold/20 shrink-0 shadow-[0_0_15px_rgba(216,182,122,0.15)] animate-pulse">
          ...
          <div className="grid gap-12 lg:grid-cols-12 items-center">
            ...
            <div className="lg:col-span-5 flex justify-center items-center">
              <Reveal>
                ...
                >
                  {/* Sapphire aura glow behind illustration */}
                  <div className="absolute -inset-4 bg-gradient-to-tr from-sapphire/25 to-gold/20 rounded-[2rem] blur-3xl opacity-60 group-hover/faq-img:opacity-85 transition-opacity duration-700 pointer-events-none" />

                  {/* Gold-sapphire rotating orbit border frame */}
                  <div className="absolute -inset-2.5 rounded-[2rem] bg-gradient-to-br from-gold/30 via-sapphire/20 to-gold/15 blur-sm opacity-40 animate-spin-slow group-hover/faq-img:opacity-60 transition-all duration-700 -z-10" />
        ...
        {success && (
          ...
          >
            ...
            >
              ...
              <h3 className="mt-5 font-display text-3xl text-gradient-gold">Message Sent</h3>
              <p className="mt-3 text-sm text-muted-foreground leading-relaxed">
                Your message has been received with gratitude. Shivani will read it and reply personally within 24 hours.
              </p>
              <div className="mt-5 flex items-center justify-center gap-2 text-xs uppercase tracking-[0.25em] text-gold">
                <Sparkles className="h-3 w-3 animate-pulse" /> Cosmic blessings on your journey
              </div>
